...
from mylab.trainer import train, evaluate, test_infer_time
...
def get_dataset():
    ...
    return train_sutd, val_sutd, test_sutd

def get_data_loader(test_sutd):
...
class InferenceTime:
    def __init__(self):
        train_sutd, val_sutd, test_sutd = get_dataset()
        self.test_sutd = test_sutd

    def __call__(self, model_name):
        print(model_name)
        dataloader = get_data_loader(self.test_sutd)
        model, loss_fn, opt, early_stop_s, lr_s, metrics = get_model()
        return test_infer_time(model, dataloader, args)
